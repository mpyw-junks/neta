...
imagebwbmp(imagecreatefrompng('src.png'), 'out.bmp', 0.45);
...
function imagebwbmp($image, $to = null, $threshold = 0.5)
{
    ...
    fwrite($to, pack('VVVvvVVVVVV', 40, $biWidth, $biHeight,
                        1, 1, 0, $biSizeImage, 0, 0, 0, 0));
    fwrite($to, "\xff\xff\xff\x00"); // white
    fwrite($to, "\x00\x00\x00\x00"); // black
    
    for ($y = $biHeight - 1; $y >= 0; --$y) {

        $byte = $bitIdx = $byteIdx = 0;
        ...
        for ($x = 0; $x < $biWidth; ++$x) {
            ...
            // Append color bit to the byte and increment bit index
            $byte = ($byte << 1) | $color;
            $bitIdx = ($bitIdx + 1) % 8;
